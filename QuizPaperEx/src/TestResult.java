...
public class TestResult extends JFrame {
	...
	private void init() {
		...
		btnBack.setBackground(new Color(234, 234, 234));
		btnBack.setFocusPainted(false);
		btnGradeCheck=new JButton("성적조회");
		btnGradeCheck.setBackground(new Color(234, 234, 234));
	...
	private void addListeners() {
		MouseListener ml = new MouseAdapter() {
			...
			public void mousePressed(MouseEvent me) {
				if (me.getSource() == lblLogout) {
					...
					new MyPage(login);
					dispose();
				}else if (me.getSource() == btnGradeCheck) {
					login.savedidExamInfo(didExam);
					...
					for (int i = 0; i < quizSolve.length; i++) {						
						if (me.getSource() == quizSolve[i]) {												
							new QuizSolve(TestResult.this, p, getDidsubjectSolv(i),getSubs());
							quizSolve[i].setBorder(new LineBorder(Color.WHITE));
							dispose();
						}
					}
					...
					for (int i = 0; i < checkWrong.length; i++) {
						if (me.getSource() == checkWrong[i]) {
							if (getWrongItems(i).size() > 0) {
								ansidx=i;
								new WrongCheck(getWrongItems(i), TestResult.this, p, getDidsubjectSolv(i),getSubs());
								checkWrong[i].setBorder(new LineBorder(Color.WHITE));
								dispose();
							} else {
		...
		btnGradeCheck.addMouseListener(ml);
	...
	public Vector<Integer> getWrongItems(int n) {
	...
	public QuizAnswer[] getQuizAnswerList(int n) {
		Vector<DidSubject> didSubList = didExam.getSubjectList();
		return didSubList.get(n).getAnswers();
	}
	private Vector<String> getSubs(){
		Vector<DidSubject> subjects=didExam.getSubjectList();
